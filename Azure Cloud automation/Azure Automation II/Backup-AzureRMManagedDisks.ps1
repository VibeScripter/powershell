...
foreach ($mDisk in $mDisks) {
    ...
    $copyBlob = Start-AzureStorageBlobCopy -AbsoluteURL $mDiskSAS.AccessSAS -DestContainer $containerName -DestBlob $blobName -DestContext $Context -ErrorAction $eAction
